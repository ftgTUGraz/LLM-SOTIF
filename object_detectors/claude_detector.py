...
"""
Description:
    This script processes a directory of images, sends them to the Anthropic
    Claude API to detect objects (cars, pedestrians, signs, etc.), and assesses
...
Usage:
    python claude_detector.py --input <path_to_images> --output <path_to_results> [options]
...
Arguments:
    ...
    -m, --model       Claude model version (default: claude-3-7-sonnet-20250219)
    -k, --key         Anthropic API Key (optional if ANTHROPIC_API_KEY env var is set)
...
def encode_image(image_path):
    """
    Encodes an image file to a base64 string required by Anthropic API.
    """
    with open(image_path, "rb") as image_file:
        return base64.b64encode(image_file.read()).decode('utf-8')


def call_claude(client, image_path, prompt, model, max_retries=3):
    """
    Calls the Claude API with exponential backoff for retries.
    """
    retries = 0
    ...
    while retries < max_retries:
        try:
            ...
            response = client.messages.create(
                model=model,
                max_tokens=2000,  # Increased to accommodate thinking + json
            ...
            if retries >= max_retries:
                print(
                    Fore.RED + Style.BRIGHT + f"[X] Claude API call failed after {max_retries} retries. Final error: {e}")
                return None
...
def parse_and_save_response(raw_data, output_path):
    """
    Parses the raw JSON string from Claude, validates the schema,
    and writes the formatted .txt file.
    """
    try:
        # regex to find the JSON array in case the model adds text around it
...
# -----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Claude Dashcam Analyzer")
    ...
    parser.add_argument("--model", "-m", default="claude-3-7-sonnet-20250219", help="Claude Model ID")
    parser.add_argument("--limit", "-l", type=int, default=1000, help="Max number of images to process")
    parser.add_argument("--key", "-k", help="Anthropic API Key (Optional if env var is set)")

    args = parser.parse_args()

    # setup API key
    api_key = args.key or os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("CLAUDE_API_KEY")
    if not api_key:
        print(
            Fore.RED + "Error: API Key not found. Please set ANTHROPIC_API_KEY environment variable or use --key flag.")
        return
    ...
    for root, _, files in os.walk(input_root):
        ...
        for file in image_files:
            ...
            raw_data = call_claude(client, input_file_path, USER_PROMPT, args.model)
