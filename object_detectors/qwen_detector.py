...
"""
Description:
    This script processes a directory of images, sends them to the Alibaba Cloud
    Qwen Vision API (via DashScope) to detect objects (cars, pedestrians, signs,
    etc.), and assesses if they are safety-critical. The output is saved as a
    structured text file compatible with label formats (Class X Y W H Safety).

    It recursively scans the input directory and mirrors the folder structure
    in the output directory.

Usage:
    python qwen_detector.py --input <path_to_images> --output <path_to_results> [options]
...
Arguments:
    ...
    -m, --model       Qwen model version (default: qwen-vl-max).
    -k, --key         Qwen API Key (optional if QWEN_API_KEY env var is set).
...
def call_qwen(client, image_path, prompt, model, max_retries=3):
    """
    Calls the Qwen Vision API with exponential backoff for retries.
    """
    retries = 0
    ...
    while retries < max_retries:
        try:
            ...
            if retries >= max_retries:
                print(
                    Fore.RED + Style.BRIGHT + f"[X] Qwen API call failed after {max_retries} retries. Final error: {e}")
                return None
...
def parse_and_save_response(raw_data, output_path):
    """
    Parses the raw JSON string from Qwen, validates the schema,
...
# -----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Qwen Dashcam Analyzer")
    ...
    parser.add_argument("--model", "-m", default="qwen-vl-max", help="Qwen Model ID")
    parser.add_argument("--limit", "-l", type=int, default=1000, help="Max number of images to process")
    parser.add_argument("--key", "-k", help="Qwen API Key (Optional if env var is set)")

    args = parser.parse_args()

    # setup API key
    api_key = args.key or os.environ.get("QWEN_API_KEY")
    if not api_key:
        print(Fore.RED + "Error: API Key not found. Please set QWEN_API_KEY environment variable or use --key flag.")
        return

    # client with Alibaba DashScope Base URL
    client = OpenAI(
        api_key=api_key,
    ...
    for root, _, files in os.walk(input_root):
        ...
        for file in image_files:
            ...
            raw_data = call_qwen(client, input_file_path, USER_PROMPT, args.model)
